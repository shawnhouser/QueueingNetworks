...
Suppose you want to solve the flow balance equation for a birth-death type queue with finite capacity $n$. Given an array of arrival (birth) rates $\lambda = (\lambda_{0}, \lambda_{1}, \cdots)$ and an array of service (death) rates $\mu = (\mu_{1}, \mu_{2}, \cdots)$, the function BD.transitions($\lambda$, $\mu$) will return the transition probabilities for the system. Please see the following example with $\lambda = (3, 2, 1)$ and $\mu = (1, 2, 2)$.

```{r BD.transitions, echo=FALSE }
...
```{r ex7}
q <- mmc.summary(lambda = 4, mu = 2, c = 3, plot_transitions = TRUE)
