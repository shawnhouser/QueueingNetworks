...
gg1.summary <- function(arrivals, service_times, interarrival = FALSE) {
  ...
  if (interarrival == TRUE) {
    ...
    df <- data.frame(
      customers = c(0, seq(1:n)), arrival_times = arrival_times,
    ...
    return(list(
      "bookkeeping" = df, "res" = res, "lambda" = lambda,
...
mmc.summary <- function(lambda, mu, c,
  ...
  if (c == 1) {
    ...
    if (plot_pn == TRUE) {
      num_ent <- c(0, 1:(c - 1))
      plot(num_ent, pn,
        main = "Distribution of Number of Customers in System",
    ...
    if (plot_waitSys == TRUE) {
      plot(t, waitSys_probabilities,
        main = "Waiting Time in System",
    ...
    if (plot_waitQ == TRUE) {
      plot(c(0, t), waitQ_probabilities,
        main = "Waiting Time in Queue",
...
## ----ex3----------------------------------------------------------------------
q <- mmc.summary(
  lambda = 4, mu = 4.4545, c = 1, plot_pn = TRUE,
  plot_waitQ = TRUE,
  plot_waitSys = TRUE
)
library(gt)
